...
# Loop through all entries until user is done 'xxx'
while True:
    ...
    # Add all entered values to their respective lists
...
bought_num = 0
# Loop through all ingredient names in the list and get bought information
for item in ingre_names:
    ...
    # Set buy_unit_list to only units that work with corresponding unit entered earlier
...
for item in ingre_names:
    ...
    print(f"ingre_amounts_fix: {ingre_amounts_fix[iterate_num]}")
    print(f"bought_amounts_fix: {bought_amounts_fix[iterate_num]}")
...
# Set total to the first item (In case there is only one)
total_ingre_cost = ingredient_costs[0]
# If there is more than one ingredient calculate total
while cost_length > 1:
...
# Dictionary containing name of columns and lists for Pandas Table
recipe_cost_dict = {
...
go_back = os.path.dirname(os.path.dirname( __file__ ))
save_path = go_back + '\Written Files'
file_name = f"{recipe_name}.txt"
complete_name = os.path.join(go_back, save_path, file_name)

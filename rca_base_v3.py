        ...
        try:
            ...
            if response <= 0:
                if zero == False:
                    ...
                    check = string_checker(f"Are you sure you mean '{response}'? ", y_n_list, 1)
                    if check == "yes":
                        return response
                    else:
                        print(error)
...
def instructions_display():
    print("---PLACEHOLDER FOR INSTRUCTIONS---")
    print()
...
while cost_length > 1:
    total_ingre_cost = total_ingre_cost + ingredient_costs[cost_length - 1]
    cost_length -= 1

total_ingre_cost = '%.2f' % total_ingre_cost
print(f"Total ingredient Cost: ${total_ingre_cost}")

per_serving = '%.2f' % (total_ingre_cost / recipe_servings)
print(f"Cost Per Serving: ${per_serving}")